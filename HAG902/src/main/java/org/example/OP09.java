...
public class OP09 extends  ChatMA01{
    OP09(){
        ...
        ChatRequest request = ChatRequest.builder().messages(new AiMessage(t.content())).

        parameters(DefaultChatRequestParameters.builder().modelName("Andy").maxOutputTokens(100).

        toolChoice(ToolChoice.AUTO).responseFormat(JsonSchema.builder().name("JSon").build()).build()).build();
